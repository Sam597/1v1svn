...
class Pform extends Think {
    ...
    public function image($info, $value) {
        ...
        $parseStr = '<a href="'.$value.'" target="_blank"><img src="'.$value.'" width:50px;height:50px /></a>';
        return $parseStr;
    }
    ...
    public function images($info, $value) {
        ...
        if ($value) {
            ...
            if (is_array($options)) {
                foreach ($options as $r) {
                    ...
                    $k = trim($v[1]);
                    $optionsarr[$k] = $v[0];
                    $data .='<div style="padding-bottom:10px;border-bottom:1px solid #e2e2e2;margin-bottom:10px" id="uplistd_' . $i . '"> 
                        <img src="' . $v[0] . '" width="100" height="100" />    
                <input type="text" size="50" class="input-text" name="' . $field . '[]" value="' . $v[0] . '"  /> <input type="text" class="input-text" name="' . $field . '_name[]" value="' . $v[1] . '" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplistd_' . $i . '\');">' . L('remove') . '</a> </div>';
                    $i++;
                }
            }
        }
		...
		<div id="' . $field . '_images" class="imagesList">
                <input type="hidden"  name="' . $field . '[]" value=""/><input type="hidden"   name="' . $field . '_name[]" value="" />' . $data . '</div>
		</fieldset>
		<div class="c"></div>
