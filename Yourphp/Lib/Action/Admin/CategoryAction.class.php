...
class CategoryAction extends AdminbaseAction {
    ...
    public function index() {
        if ($this->categorys) {
            foreach ($this->categorys as $r) {
                $MENU=M('Menu');
                $m=$MENU->field(true)->where(array('action'=>'index','model'=>$r['module']))->find();
                $r['menuid']=$m['id'];
                if ($r['module'] == 'Page') {
                    ...
                    $r['str_manage'] = '<a href="?g=Admin&m=' . $r['module'] . '&a=index&catid=' . $r['id'] . '&menuid='.$r['menuid'].'">' . '查看内容' . '</a> | <a href="?g=Admin&m=' . $r['module'] . '&a=add&catid=' . $r['id'] . '">' . L('add_content') . '</a> | ';
                }
    ...
    public function delete() {
        ...
        if ($this->categorys[$catid]['type'] == 1) {
            ...
            $moduleid = $this->mod[($this->categorys[$catid]['module'])];
