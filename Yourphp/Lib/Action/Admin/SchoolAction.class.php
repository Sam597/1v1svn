...
class SchoolAction extends AdminbaseAction {
    ...
    public function index() {
        $school = M('School');
        $pid = $_GET['parentid'];
        $schoollist = $school->where('parentid=' . $pid)->select();
    ...
    public function add() {
        $area = M('Area');
        $pid = $_GET['parentid'];
        $areaname = $area->getFieldbyTypeid($pid, 'name');
        $siteid = $area->getFieldbyTypeid($pid, 'parentid');
    ...
    public function _before_add() {
        ...
        if ($parentid) {
            foreach ((array) $this->Type as $key => $r) {
                if ($r['keyid'] != $keyid || empty($r['status']))
                    continue;
                $r['id'] = $r['typeid'];
                $array[] = $r;
            }
