...
class IndexAction extends AdminbaseAction {
    ...
    function _initialize() {
        ...
        $signup = M('Signup');
        $feedback = M('Feedback');
        $reg_count = $signup->where(array('status' => 0,'lang'=>1))->count();
        $feedback_count = $feedback->where(array('status' => 0))->count();
    ...
    public function main() {
        ...
        $yourphp_info = array(
            'yourphp_VERSION' => VERSION . ' ' . UPDATETIME . ' [ <a href="http://www.duoyan.net" target="_blank">' . L('view_new_VERSION') . '</a> ]',
