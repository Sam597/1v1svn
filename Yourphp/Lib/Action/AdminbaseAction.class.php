...
//			$map['siteid|siteid'] = array(0, $_SESSION['siteid'],'_multi'=>true);
//			$map['schoolid|schoolid'] = array(0, $_SESSION['schoolid'],'_multi'=>true);
            $map['siteid'] = $_SESSION['siteid'];
            $map['schoolid'] = $_SESSION['schoolid'];
        }
        $this->assign($_REQUEST);

        //校区查询
        $S_siteid = $this->_request('siteid');
        $S_schoolid = $this->_request('schoolid');
        if ($S_siteid) {
            $map['siteid'] = $S_siteid;
        }
        if ($S_schoolid) {
            $map['schoolid'] = $S_schoolid;
        }
        ...
        $course_tbl = M('Course');
        $site_tbl = M('Site');
        $term_id = $this->_request('term_id');
        $grade_id = $this->_request('grade_id');
        $course_id = $this->_request('course_id');
        $select_siteid = $this->_request('siteid');
        $area_id = $this->_request('areaid');
        ...
        if ($select_siteid) {
            $map['siteid'] = $select_siteid;
        }
        if ($area_id) {
            $map['areaid'] = $area_id;
        }

        $term_list = $term_tbl->select();
        $this->assign('term_list', $term_list);

        $grade_list = $grade_tbl->select();

        if ($grade_id) {
            $course_list = $course_tbl->where(array('grade_id' => $grade_id))->select();
        } else {
            $course_list = array();
        }

        if ($select_siteid) {
            $area_list = $site_tbl->where(array('siteid' => $select_siteid, 'level' => 2))->select();
        } else {
            $area_list = array();
        }
        ...
        if ($area_id) {
            $school_list = $site_tbl->where(array('areaid' => $area_id, 'level' => 3))->select();
        } else {
            $school_list = array();
        }
        $this->assign('school_list', $school_list);
        if ('Feedback' == $modelname || 'Signup' == $modelname) {
            unset($map['schoolid']);
        }
        if ('Teacher' == $modelname) {
            $course_list = array(
                0 => array('name' => '数学', 'id' => 1),
                1 => array('name' => '语文', 'id' => 2),
                2 => array('name' => '英语', 'id' => 3),
                3 => array('name' => '物理', 'id' => 4),
                4 => array('name' => '化学', 'id' => 5),
                5 => array('name' => '生物', 'id' => 6),
                6 => array('name' => '政治', 'id' => 7),
                7 => array('name' => '地理', 'id' => 8),
                8 => array('name' => '历史', 'id' => 9)
            );
            $grade_list = array(
                0 => array('name' => '小学', 'id' => 1),
                1 => array('name' => '初中', 'id' => 2),
                2 => array('name' => '高中', 'id' => 3)
            );
            if ($grade_id) {
                unset($map['course_id']);
                $map['_string'] = "find_in_set(" . $grade_id . ",grade_ids)";
            }
            if ($course_id) {
                unset($map['course_id']);
                $map['_string'] = "find_in_set(" . $course_id . ",course_ids)";
            }
            if ($grade_id && $course_id) {
                $map['_string'] = "find_in_set(" . $grade_id . ",grade_ids) and find_in_set(" . $course_id . ",course_ids)";
            }
        ...
        if ($count > 0) {
			...
			if ($term_id)
                $page->parameter .= "&term_id=" . $term_id . "&";
            if ($grade_id)
                $page->parameter .= "&grade_id=" . $grade_id . "&";
            if ($course_id)
                $page->parameter .= "&course_id=" . $course_id . "&";
            if ($area_id)
                $page->parameter .= "&areaid=" . $area_id . "&";
            if ($order)
                $page->parameter .= "&order=" . $order . "&";
            if ($sort)
                $page->parameter .= "&sort=" . $sort . "&";
            $site = M('Site');
            foreach ($voList as $k => $v) {
                if ($v['siteid'])
                    $voList[$k]['sitename'] = $site->getfieldbyid($v['siteid'], 'name');
                if ($v['schoolid'])
                    $voList[$k]['schoolname'] = $site->getfieldbyid($v['schoolid'], 'name');
            }
            $map[C('VAR_PAGE')] = '{$page}';
            unset($map['lang']);
            $map['lang'] = LANG_ID;
