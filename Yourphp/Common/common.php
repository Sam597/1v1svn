...
function getsite($id) {
    $site = M('Site');
    return $site->getFieldById($id, 'name');
}

function getcourse($course_id) {
    $arr = explode(',', $course_id);
    $course = array(1 => '数学', 2 => '语文', 3 => '英语', 4 => '物理', 5 => '化学', 6 => '生物', 7 => '政治', 8 => '地理', 9 => '历史');
...
function getstudenttype($type_id) {
    ...
    return $array[$type_id] ? $array[$type_id] : '';
}
...
function get_teachername($id, $field) {
    $module = M('Teacher');
...
//获取年级
function get_grade($id, $field) {
    $grade = M('Grade');
    $result = $grade->where('id = ' . $id)->order('listorder')->find();
    return $result[$field] ? $result[$field] : '';
}

//获取年级
function get_course($id, $field) {
    $course = M('Course');
    $result = $course->where('id = ' . $id)->order('listorder')->find();
    return $result[$field] ? $result[$field] : '';
}

//获取年级
function get_sitename($id, $field) {
    $site = M('Site');
    $result = $site->where('id = ' . $id)->order('listorder')->find();
    return $result[$field] ? $result[$field] : '';
}
